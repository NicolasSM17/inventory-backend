...
public class CategoryServiceImpl implements ICategoryService{
	...
	public ResponseEntity<CategoryResponseRest> save(Category category) {
		...
		try {
			...
			response.setMetadata("Respuesta erronea", "-1", "Error al grabar categoria");
			e.printStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
		...
		try {
			...
			if(categorySearch.isPresent()) {
				...
				if(categoryToUpdate != null) {
					...
					response.setMetadata("Respuesta ok", "-1", "Categoria no actualizada");
					return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.BAD_REQUEST);
				}
			} else {
				response.setMetadata("Respuesta erronea", "-1", "Categoria no encontrada");
			...
			response.setMetadata("Respuesta erronea", "-1", "Error al actualizar categoria");
			e.printStackTrace();
			return new ResponseEntity<CategoryResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
